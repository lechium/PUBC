//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "TWTRTweetEntity.h"

#import "NSCoding.h"
#import "TWTRJSONConvertible.h"

@class NSString;

@interface TWTRTweetUrlEntity : TWTRTweetEntity <NSCoding, TWTRJSONConvertible>
{
    NSString *_displayUrl;	// 8 = 0x8
    NSString *_expandedUrl;	// 16 = 0x10
    NSString *_url;	// 24 = 0x18
}

@property(readonly, copy, nonatomic) NSString *url; // @synthesize url=_url;
@property(readonly, copy, nonatomic) NSString *expandedUrl; // @synthesize expandedUrl=_expandedUrl;
@property(readonly, copy, nonatomic) NSString *displayUrl; // @synthesize displayUrl=_displayUrl;
- (void).cxx_destruct;	// IMP=0x0000000100838744
- (id)accessibilityValue;	// IMP=0x0000000100838708
- (void)encodeWithCoder:(id)arg1;	// IMP=0x00000001008385d8
- (id)initWithCoder:(id)arg1;	// IMP=0x0000000100838478
- (id)initWithJSONDictionary:(id)arg1;	// IMP=0x00000001008382fc

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

